...
class GPTClient:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a digital marketing expert specializing in campaign performance analysis and optimization."
                    },
                    {
                        "role": "user",
                        "content": full_prompt
    ...
    def _generate_basic_insights(self, df):
        ...
        insights.append(f"- **Total Campaigns:** {df['campaign_name'].nunique()}")
        insights.append(f"- **Total Records:** {len(df)}")
        insights.append(f"- **Date Range:** {df['date'].min().date()} to {df['date'].max().date()}")
        insights.append(f"- **Total Spend:** ₹{df['spend'].sum():,.2f}")
        insights.append(f"- **Total Impressions:** {df['impressions'].sum():,}")
        insights.append(f"- **Total Clicks:** {df['clicks'].sum():,}")
        
        if 'purchases' in df.columns:
            insights.append(f"- **Total Conversions:** {df['purchases'].sum():,}")
        ...
        insights.append(f"- **Average CTR (Click-Through Rate):** {avg_ctr:.2%}")
        insights.append(f"- **Average CPC (Cost Per Click):** ₹{avg_cpc:.2f}")
        insights.append(f"- **Average CPM (Cost Per 1000 Impressions):** ₹{avg_cpm:.2f}")
        
        if 'purchases' in df.columns:
            avg_cvr = df['cvr'].mean() if 'cvr' in df.columns else 0
            avg_cpa = df['cpa'].mean() if 'cpa' in df.columns else 0
            insights.append(f"- **Average CVR (Conversion Rate):** {avg_cvr:.2%}")
            insights.append(f"- **Average CPA (Cost Per Acquisition):** ₹{avg_cpa:.2f}")
        else:
            avg_cvr = 0
        ...
        if avg_ctr < 0.01:  # Less than 1%
            insights.append("- **Improve Ad Creative**: CTR is below 1%, consider testing new visuals and copy")
        
        if avg_cpc > 50:  # High CPC for INR
            insights.append("- **Optimize Targeting**: High CPC suggests need for better audience targeting")
        
        if 'purchases' in df.columns:
            if df['purchases'].sum() == 0:
                insights.append("- **Conversion Tracking**: No conversions detected, verify tracking setup")
            elif avg_cvr < 0.01:
                insights.append("- **Landing Page Optimization**: Low conversion rate suggests landing page improvements needed")
        
        insights.append("- **Budget Optimization**: Focus budget on top-performing campaigns")
        insights.append("- **A/B Testing**: Implement systematic testing of ad variations")
        insights.append("- **Performance Monitoring**: Set up regular performance review cycles")
